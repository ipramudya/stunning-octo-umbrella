...
import { AttendanceConflict, AttendanceZoneRepository } from "../src/attendance-zone.js";
import type { OracleDatabase } from "../src/oracle.js";

function repository(execute: ReturnType<typeof vi.fn>, commit = vi.fn(), rollback = vi.fn()) {
  const connection = { execute, commit, rollback };
  const database = {
    withTransaction: async (work: (value: typeof connection) => Promise<unknown>) => {
      try {
  ...
  return {
    repository: new AttendanceZoneRepository(database as unknown as OracleDatabase),
    commit,
    rollback,
  };
}
...
describe("attendance mutation transaction", () => {
  it("locks the zone before ordered entries and commits", async () => {
    ...
    await expect(
      zones.withAttendanceMutation("employee", new Date("2026-09-08T00:00:00Z"), async () => 7),
    ).resolves.toBe(7);

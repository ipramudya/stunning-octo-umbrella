...
export class ManualDecisionRepository {
  ...
  decide(input: {
    ...
    return this.database.withTransaction(async (connection) => {
      ...
      if (!target) throw new Error('ATTENDANCE_ENTRY_NOT_FOUND');
      await connection.execute(
      ...
      if (!locked) throw new Error('ATTENDANCE_ENTRY_NOT_FOUND');
      ...
      if (!updated) throw new Error('ATTENDANCE_ENTRY_NOT_FOUND');
  ...
  private async requireClockIn(
    ...
    if (!occurredAt) throw new Error('CLOCK_IN_REQUIRED');

...
import {
  ...
  validateManualAttendancePolicy,
...
describe('manual attendance', () => {
  ...
  it('applies Jakarta date, seven-day, and future-time policy', () => {
      ...
      validateManualAttendancePolicy(
        {
          ...request,
          workDate: '2026-08-31',
          claimedAt: new Date('2026-08-31T01:00:00Z'),
        },
        now,
      ),
    ).toThrowError(
      expect.objectContaining({ code: 'MANUAL_DATE_OUT_OF_RANGE' }),
    );
    expect(() =>
      validateManualAttendancePolicy(
        { ...request, workDate: '2026-02-31' },
        now,
      ),
    ).toThrowError(expect.objectContaining({ code: 'VALIDATION_ERROR' }));
    expect(() =>
      validateManualAttendancePolicy(
        { ...request, claimedAt: new Date('2026-09-08T04:00:00Z') },
        now,
      ),
    ).toThrowError(expect.objectContaining({ code: 'FUTURE_CLAIMED_AT' }));
    expect(() =>
      validateManualAttendancePolicy(
        { ...request, claimedAt: new Date('2026-09-07T16:00:00Z') },
        now,
      ),
    ).toThrowError(
      expect.objectContaining({ code: 'CLAIMED_AT_DATE_MISMATCH' }),
    );
  });

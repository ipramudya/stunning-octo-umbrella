...
describe("identity gRPC health", () => {
  it("maps dependency readiness to the standard serving status", async () => {
    const isReady = vi.fn().mockResolvedValueOnce(true).mockResolvedValueOnce(false);
    const controller = new GrpcHealthController({ isReady } as unknown as ReadinessService);

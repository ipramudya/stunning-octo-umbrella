...
const otherCreation = await fetch(`${baseUrl}/api/v1/hrd/employees`, {
  method: "POST",
  headers: { cookie: cookieHeader(hrd), "content-type": "application/json", origin },
  body: JSON.stringify({
...
async function authorize(jar, body) {
  return fetch(`${baseUrl}/api/v1/me/evidence-uploads`, {
    method: "POST",
    headers: { cookie: cookieHeader(jar), "content-type": "application/json", origin },
...
assert.equal((await authorize(employee, { contentType: "text/plain", sizeBytes: 8 })).status, 400);
assert.equal(
  (await authorize(employee, { contentType: "image/png", sizeBytes: 5 * 1024 * 1024 + 1 })).status,
  400,
);
const authorized = await authorize(employee, { contentType: "image/png", sizeBytes: png.length });
...
for (const jar of [employee, hrd]) {
  ...
  assert.deepEqual(Buffer.from(await (await fetch(value.url)).arrayBuffer()), png);
}
assert.equal((await access(other, upload.uploadId)).status, 404);

const wrong = await (await authorize(employee, { contentType: "image/png", sizeBytes: 8 })).json();
...
await fetch(expired.url, { method: "PUT", headers: expired.headers, body: png });
const expire = `
...
finalizeUpload(expired.uploadId, "EVIDENCE_EXPIRED");

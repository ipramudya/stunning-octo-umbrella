...
for (const [body, code] of [
  ...
  await expectProblem(
    await request('/api/v1/hrd/employees', 'POST', hrd, body),
    409,
    code,
  );
}

const searchedResponse = await request(
  '/api/v1/hrd/employees?q=ticket&limit=1',
  'GET',
  hrd,
);
...
await expectProblem(
  await request(`/api/v1/hrd/employees/${created.id}`, 'PATCH', hrd, {
